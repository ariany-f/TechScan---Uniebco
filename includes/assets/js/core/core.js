      ...
      if (hashtags !== null && hashtags.length > 0) {
        ...
        $.post(api['data/search'], { 'query': query }, function (response) {
          if (response.callback) {
            ...
            $('#search-results-all').attr('href', site_path + '/comunidade/search/' + query);
          } else {
            $('#search-results .dropdown-widget-header').hide();
            $('#search-results-all').hide();
  ...
  $('body').on('keydown', '#search-input', function (event) {
    if (event.keyCode == 13) {
      ...
      if (!is_empty(query)) {
        ...
        if (hashtags !== null && hashtags.length > 0) {
          var query = hashtags[0].replace("#", "");
          window.location = site_path + '/comunidade/search/hashtag/' + query
        } else {
          window.location = site_path + '/comunidade/search/' + query
        }
      }
      return false;
    }
    ...
    if (!is_empty(query)) {
      ...
      if (handle !== undefined) {
        ...
        if (hashtags !== null && hashtags.length > 0) {
          ...
          window.location = site_path + '/comunidade/search/' + query + filter;
        }
      }
    }
    return false;
