...
<div class="page-header">
  {if $view == "share"}
    <img class="floating-img d-none d-md-block" src="{$system['system_url']}/comunidade/content/themes/{$system['theme']}/images/headers/undraw_share_766i.svg">
  {else}
    <img class="floating-img d-none d-md-block" src="{$system['system_url']}/comunidade/content/themes/{$system['theme']}/images/headers/undraw_code_typing_7jnv.svg">
  {/if}
...
<div class="container offcanvas" style="margin-top: -25px;">
  <div class="row">
        ...
        <ul>
          {if $system['developers_apps_enabled']}
            <li {if $view == ""}class="active" {/if}>
              <a href="{$system['system_url']}/comunidade/developers"><i class="fa fa-book mr5"></i>{__("Documentation")}</a>
            </li>
          {/if}
          {if $user->_logged_in && $system['developers_apps_enabled']}
            <li {if $view == "apps" || $view == "new" || $view == "edit"}class="active" {/if}>
              <a href="{$system['system_url']}/comunidade/developers/apps"><i class="fa fa-cubes mr5"></i>{__("My Apps")}</a>
            </li>
          {/if}
          {if $system['developers_share_enabled']}
            <li {if $view == "share"}class="active" {/if}>
              <a href="{$system['system_url']}/comunidade/developers/share"><i class="fa fa-share-alt-square mr5"></i>{__("Share Plugin")}</a>
            </li>
          {/if}
        </ul>
      </div>
      ...
      {if $view == ""}
        ...
        <div class="card mt20">
          ...
          <div class="card-body page-content pt30" style="font-size: 1.1em;">
            ...
            <p>
              {__("In order for your app to access our APIs, you must register your app using the")} <a href="{$system['system_url']}/comunidade/developers/apps">{__("App Dashboard")}</a>. {__("Registration creates an App ID that lets us know who you are, helps us distinguish your app from other apps")}.
            </p>
            <ol>
              <li class="mb20">
                {__("You will need to create a new App")}
                <a class="btn btn-sm btn-primary ml10" href="{$system['system_url']}/comunidade/developers/new">
            ...
            <ol>
              <li class="mb20">
                ...
                <pre class="mtb10">&lt;a href="{$system['system_url']}/comunidade/api/oauth?app_id=YOUR_APP_ID"&gt;Log in With {__($system['system_title'])}&lt;/a&gt;</pre>
                <p style="font-size: 15px;">
                  {__("The user will be redirect to Log in With page like this")}
                </p>
                <div class="text-center">
                  <img class="img-fluid" width="400" src="{$system['system_url']}/comunidade/content/themes/{$system['theme']}/images/screenshots/login_with.png">
                </div>
              </li>
              <li class="mb20">
    ...
    $get = file_get_contents("{$system['system_url']}/comunidade/api/authorize?app_id=$app_id&app_secret=$app_secret&auth_key=$auth_key");
    ...
    ?&gt;</pre>
       ...
       $access_token = $json['access_token']; // your access token
       $get = file_get_contents("{$system['system_url']}/comunidade/api/get_user_info?access_token=$access_token");
    }</pre>
            <p>
      ...
      {elseif $view == "apps"}
        ...
        <div class="card mt20">
          <div class="card-header with-icon">
            <a class="btn btn-sm btn-primary float-right" href="{$system['system_url']}/comunidade/developers/new">
              <i class="fa fa-plus-circle mr5"></i>{__("Create New App")}
            </a>
                  ...
                  <tbody>
                    {foreach $apps as $app}
                      <tr>
                        <td>{$app@iteration}</td>
                        <td>
                          <a target="_blank" href="{$system['system_url']}/comunidade/developers/edit/{$app['app_auth_id']}">
                        ...
                        <td>
                          <a data-toggle="tooltip" data-placement="top" title='{__("Edit")}' href="{$system['system_url']}/comunidade/developers/edit/{$app['app_auth_id']}" class="btn btn-sm btn-icon btn-rounded btn-primary">
        ...
        <div class="card mt20">
          <div class="card-header with-icon">
            ...
            <div class="float-right">
              <a href="{$system['system_url']}/comunidade/developers/apps" class="btn btn-sm btn-light">
      ...
      {elseif $view == "share"}
        ...
        function SocialShare(url) {
            window.open('{$system['system_url']}/comunidade/share?url=' + url, '', 'height=600,width=800');
        }
    &lt;/script&gt;</pre>
            <h6>
