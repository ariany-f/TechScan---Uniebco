{if !$user->_logged_in}
    ...
    <body data-view="{$page}" data-hash-tok="{$session_hash['token']}" data-hash-pos="{$session_hash['position']}" class="{if $system['theme_mode_night']}night-mode{/if} visitor n_chat {if ($page == 'index' && !$system['newsfeed_public']) || $page == 'sign'}index-body{/if}" {if ($page == 'index' || $page == 'sign') && !$system['system_wallpaper_default'] && $system['system_wallpaper']} style="background-image: url('{$system['system_uploads']}/{$system['system_wallpaper']}'); background-size: cover;" {/if} {if $page == 'profile' && $system['system_profile_background_enabled'] && $profile['user_profile_background']}style="background: url({$profile['user_profile_background']}) fixed !important; background-size: 100% auto;" {/if}>
  {else}
      ...
      {if !$system['system_live']}
        ...
        <div class="top-bar danger">
          <div class="container">
            ...
            <span>{__("Turn it on from")}</span> <a href="{$system['system_url']}/comunidade/admincp/settings">{__("Admin Panel")}</a>
          </div>
        </div>
        <!-- top-bar alert-->
      {/if}
              ...
              <div class="logo-wrapper">
                ...
                <a href="{$system['system_url']}" class="logo {if !$user->_logged_in && $system['newsfeed_public']}with-menu-icon{/if}">
                  {if $system['system_logo']}
                    ...
                    {else}
                      <img class="img-fluid" src="content/themes/default/images/home/logo_white.png" alt="{__($system['system_title'])}" title="{__($system['system_title'])}">
                    {/if}
                  {else}
                ...
                {if $user->_logged_in}
                  <!-- home-icon -->
                  <a href="/comunidade" class="home-icon">
                  ...
                  <div class="navbar-wrapper">
                    <ul class="clearfix">
                      {if $user->_logged_in}
                          ...
                          <li class="d-none d-xl-block dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                              ...
                              {if $user->_data['can_write_articles']}
                                <a class="dropdown-item" href="{$system['system_url']}/comunidade/blogs/new">
                              ...
                              {if $user->_data['can_create_ads']}
                                <a class="dropdown-item" href="{$system['system_url']}/comunidade/ads/new">
                        ...
                        <!-- search -->
                        <li class="d-block d-md-none">
                          <a href="{$system['system_url']}/comunidade/search">
                        ...
                        <li class="dropdown">
                          ...
                          <div class="dropdown-menu dropdown-menu-right">
                            {if $system['packages_enabled'] && !$user->_data['user_subscribed']}
                              <a class="dropdown-item" href="{$system['system_url']}/comunidade/packages">{__("Upgrade to Pro")}</a>
                            {/if}
                            {if $system['points_enabled'] || $system['wallet_enabled'] || $user->_data['can_raise_funding']}
                              {if $system['points_enabled']}
                                <a class="dropdown-item" href="{$system['system_url']}/comunidade/settings/points">{__("Points")}: <span class="badge badge-info">{$user->_data['user_points']}</span></a>
                              {/if}
                              {if $system['wallet_enabled']}
                                <a class="dropdown-item" href="{$system['system_url']}/comunidade/wallet">{__("Wallet")}: <span class="badge badge-success">{print_money($user->_data['user_wallet_balance']|number_format:2)}</span></a>
                              {/if}
                              {if $user->_data['can_raise_funding']}
                                <a class="dropdown-item" href="{$system['system_url']}/comunidade/settings/funding">{__("Funding")}: <span class="badge badge-success">{print_money($user->_data['user_funding_balance']|number_format:2)}</span></a>
                              {/if}
                              <div class="dropdown-divider"></div>
                            {/if}
                            <a class="dropdown-item" href="{$system['system_url']}/comunidade/{$user->_data['user_name']}">{__("Profile")}</a>
                            <a class="dropdown-item" href="{$system['system_url']}/comunidade/settings">{__("Settings")}</a>
                            <a class="dropdown-item" href="{$system['system_url']}/comunidade/settings/privacy">{__("Privacy")}</a>
                            {if $user->_is_admin}
                              <div class="dropdown-divider"></div>
                              <a class="dropdown-item" href="{$system['system_url']}/comunidade/admincp">{__("Admin Panel")}</a>
                            {elseif $user->_is_moderator}
                              <div class="dropdown-divider"></div>
                              <a class="dropdown-item" href="{$system['system_url']}/comunidade/modcp">{__("Moderator Panel")}</a>
                            {/if}
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" href="{$system['system_url']}/comunidade/signout">{__("Log Out")}</a>
                            <div class="dropdown-divider"></div>
                      ...
                      {else}
                        {if $user->_logged_in}
                          <li class="dropdown float-right">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                              <a class="dropdown-item" href="{$system['system_url']}/comunidade/signin">{__("Login")}</a>
                              {if $system['registration_enabled']}
                                <a class="dropdown-item" href="{$system['system_url']}/comunidade/signup">{__("Register")}</a>
                              {/if}
