<div class="card card-register">
    ...
    <div class="card-body">
      ...
      <form class="js_ajax-forms" data-url="core/signin.php" method="POST">
        ...
        <div class="form-group custom-control custom-checkbox mb10">
          ...
          <a class="float-right" href="{$system['system_url']}/comunidade/reset">{__("Forgotten password?")}</a>
        </div>
        <div class="form-group">
          <button type="submit" class="btn btn-block btn-login border-0 bg-gradient-login">{__("Login")}</button>
        </div>
        <!-- error -->
        ...
        {if $system['facebook_login_enabled'] || $system['google_login_enabled'] || $system['twitter_login_enabled'] || $system['linkedin_login_enabled'] || $system['vkontakte_login_enabled']}
          ...
          <div class="text-center">
            {if $system['facebook_login_enabled']}
              <a href="{$system['system_url']}/comunidade/connect/facebook" class="btn btn-sm btn-rounded btn-social-icon btn-facebook">
                <i class="fab fa-facebook-f"></i>
              </a>
            {/if}
            {if $system['google_login_enabled']}
              <a href="{$system['system_url']}/comunidade/connect/google" class="btn btn-sm btn-rounded btn-social-icon btn-google">
                <i class="fab fa-google"></i>
              </a>
            {/if}
            {if $system['twitter_login_enabled']}
              <a href="{$system['system_url']}/comunidade/connect/twitter" class="btn btn-sm btn-rounded btn-social-icon btn-twitter">
                <i class="fab fa-twitter"></i>
              </a>
            {/if}
            {if $system['linkedin_login_enabled']}
              <a href="{$system['system_url']}/comunidade/connect/linkedin" class="btn btn-sm btn-rounded btn-social-icon btn-linkedin">
                <i class="fab fa-linkedin"></i>
              </a>
            {/if}
            {if $system['vkontakte_login_enabled']}
              <a href="{$system['system_url']}/comunidade/connect/vkontakte" class="btn btn-sm btn-rounded btn-social-icon btn-vk">
                <i class="fab fa-vk"></i>
              </a>
            {/if}
          </div>
      ...
      {if $system['registration_enabled']}
        <div class="mt20 text-center mb20">
          {__("Not registered?")} <a href="{$system['system_url']}/comunidade/signup" class="js_toggle-panel text-link">{__("Create an account")}</a>
        </div>
      {/if}
    </div>
  </div>
  ...
  <div class="js_panel {if $do != 'up'}x-hidden{/if}">
    <div class="card-header">
      <img class="card-img" src="{$system['system_url']}/comunidade/content/themes/{$system['theme']}/images/home/registrar.png">
    </div>
    <div class="card-body">
      <form class="js_ajax-forms" data-url="core/signup.php" method="POST">
        ...
        <div class="form-group">
          <button type="submit" class="btn btn-login btn-block bg-gradient-login border-0">{__("Sign Up")}</button>
        </div>
        <!-- error -->
        <div class="alert alert-danger mb0 x-hidden"></div>
        <!-- error -->
      ...
      <div class="mt20 text-center">
        {__("Have an account?")} <a href="{$system['system_url']}/comunidade/signin" class="js_toggle-panel text-link">{__("Login Now")}</a>
      </div>
    </div>
  </div>
  <!-- signup -->
