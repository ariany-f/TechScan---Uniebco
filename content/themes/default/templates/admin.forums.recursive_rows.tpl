<tr class="treegrid-{$row['forum_id']} {if $row['forum_section'] != '0'}treegrid-parent-{$row['forum_section']}{/if}">
  <td>
    <a target="_blank" href="{$system['system_url']}/comunidade/forums/{$row['forum_id']}/{$row['title_url']}">
      {$row['forum_name']}
    </a>
  </td>
  <td>
  ...
  <td>
    <a data-toggle="tooltip" data-placement="top" title='{__("Edit")}' href="{$system['system_url']}/comunidade/{$control_panel['url']}/forums/edit_forum/{$row['forum_id']}" class="btn btn-sm btn-icon btn-rounded btn-primary">
