<div class="col-sm-6 col-md-4 col-lg-3">
  <div class="album-card">
    {if $album['cover']['blur']}<div class="x-blured">{/if}
      <a class="album-cover" href="{$system['system_url']}/comunidade/{$album['path']}/album/{$album['album_id']}" style="background-image:url({$album['cover']['source']});">
      </a>
      {if $album['cover']['blur']}
    </div>{/if}
    <div class="album-details">
      <a href="{$system['system_url']}/comunidade/{$album['path']}/album/{$album['album_id']}">{__($album['title'])}</a>
      <div>
