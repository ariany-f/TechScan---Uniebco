...
<div class="page-header">
  <img class="floating-img d-none d-md-block" src="{$system['system_url']}/comunidade/content/themes/{$system['theme']}/images/headers/undraw_elements_cipa.svg">
    ...
    <div class="col-md-8 col-lg-10 offcanvas-mainbar">
      ...
      <div class="content-tabs rounded-sm shadow-sm clearfix">
        <ul>
          <li {if $view == ""}class="active" {/if}>
            <a href="{$system['system_url']}/comunidade/forums"><i class="fa fa-home mr5"></i>{__("Home")}</a>
          </li>
          {if $user->_logged_in}
            <li {if $view == "my-threads"}class="active" {/if}>
              <a href="{$system['system_url']}/comunidade/forums/my-threads"><i class="fa fa-copy mr5"></i>{__("My Threads")}</a>
            </li>
            <li {if $view == "my-replies"}class="active" {/if}>
              <a href="{$system['system_url']}/comunidade/forums/my-replies"><i class="fa fa-comments mr5"></i>{__("My Replies")}</a>
            </li>
          {/if}
          <li {if $view == "search"}class="active" {/if}>
            <a href="{$system['system_url']}/comunidade/forums/search"><i class="fa fa-search mr5"></i>{__("Search")}</a>
          </li>
        </ul>
      </div>
      <!-- tabs -->
              ...
              {foreach $forum['childs'] as $_forum}
                <div class="row forum-row">
                  ...
                  <div class="col-12 col-sm-7 column">
                    <div class="mb5">
                      <strong><a href="{$system['system_url']}/comunidade/forums/{$_forum['forum_id']}/{$_forum['title_url']}">{__($_forum['forum_name'])}</a></strong>
                      ...
                      <div>
                        <ul class="sub-forums">
                          {foreach $_forum['childs'] as $__forum}
                            <li><a href="{$system['system_url']}/comunidade/forums/{$__forum['forum_id']}/{$_forum['title_url']}">{__($__forum['forum_name'])}</a>{if !$__forum@last}, {/if}</li>
                          {/foreach}
                        </ul>
                      </div>
                    {/if}
          ...
          {if $system['forums_online_enabled']}
            ...
            <div class="forum-meta-conent">
              {foreach $online_users as $_user}
                <a href="{$system['system_url']}/comunidade/{$_user['user_name']}">
        ...
        <ol class="breadcrumb forum-breadcrumb">
          ...
          {if $forum['parents']}
            {foreach array_reverse($forum['parents']) as $parent}
              <li class="breadcrumb-item"><a href="{$system['system_url']}/comunidade/forums/{$parent['forum_id']}/{$parent['title_url']}">{__($parent['forum_name'])}</a></li>
            {/foreach}
          {/if}
        ...
        <div class="forum-title">
          {if $forum['forum_section'] != '0'}
            <div class="float-right">
              <a href="{$system['system_url']}/comunidade/forums/new-thread/{$forum['forum_id']}" class="btn btn-sm btn-success">
          ...
          {if $forum['threads']}
            {foreach $forum['threads'] as $thread}
              <div class="row forum-row">
                <div class="col-12 col-sm-8 column">
                  <div class="mb5">
                    <strong><a href="{$system['system_url']}/comunidade/forums/thread/{$thread['thread_id']}/{$thread['title_url']}">{$thread['title']}</a></strong>
                  </div>
                  <div class="mb5">
                    {__("By")}: <a href="{$system['system_url']}/comunidade/{$thread['user_name']}">{$thread['user_fullname']}</a> <span class="js_moment" data-time="{$thread['time']}">{$thread['time']}</span>
                  </div>
                </div>
                <div class="col-sm-2 d-none d-sm-block column text-center">
      ...
      {elseif $view == "thread"}
        ...
        <ol class="breadcrumb forum-breadcrumb">
          ...
          {if $thread['forum']['parents']}
            ...
            {/foreach}
          {/if}
          <li class="breadcrumb-item"><a href="{$system['system_url']}/comunidade/forums/{$thread['forum']['forum_id']}/{$thread['forum']['title_url']}">{__($thread['forum']['forum_name'])}</a></li>
        ...
        <div class="forum-title clearfix">
          <div class="float-right">
            <a href="{$system['system_url']}/comunidade/forums/new-reply/{$thread['thread_id']}" class="btn btn-sm btn-primary">
        ...
        <div class="forum-thread {if $thread['replies'] > 0 && $selected_page != 1}x-hidden{/if}">
          <div class="row">
            <div class="col-12 col-sm-2 text-center">
              <a href="{$system['system_url']}/comunidade/{$thread['user_name']}"><img class="avatar" src="{$thread['user_picture']}"></a>
              <h6 class="mt10">
                <a href="{$system['system_url']}/comunidade/{$thread['user_name']}">{$thread['user_fullname']}</a>
              </h6>
              <div class="mb5">
              ...
              <div class="time clearfix">
                ...
                <div class="float-right">
                  {if $thread['manage_thread']}
                    <a data-toggle="tooltip" data-placement="top" title='{__("Edit")}' href="{$system['system_url']}/comunidade/forums/edit-thread/{$thread['thread_id']}" class="btn btn-sm btn-light btn-icon btn-rounded">
        ...
        {if $thread['replies'] > 0}
          {foreach $thread['thread_replies'] as $reply}
            <div class="forum-thread" id="reply-{$reply['reply_id']}">
              <div class="row">
                <div class="col-12 col-sm-2 text-center">
                  <a href="{$system['system_url']}/comunidade/{$reply['user_name']}"><img class="avatar" src="{$reply['user_picture']}"></a>
                  <h6 class="mt10">
                    <a href="{$system['system_url']}/comunidade/{$reply['user_name']}">{$reply['user_fullname']}</a>
                  </h6>
                  <div class="mb5">
                ...
                <div class="col-12 col-sm-10">
                  <div class="time clearfix">
                    <!-- buttons -->
                    <div class="float-right">
                      <a data-toggle="tooltip" data-placement="top" title='{__("Link")}' href="{$system['system_url']}/comunidade/forums/thread/{$thread['thread_id']}/{$thread['title_url']}#reply-{$reply['reply_id']}" class="btn btn-sm btn-light btn-icon btn-rounded">
                        <i class="fa fa-link"></i>
                      </a>
                      {if $reply['manage_reply']}
                        <a data-toggle="tooltip" data-placement="top" title='{__("Edit")}' href="{$system['system_url']}/comunidade/forums/edit-reply/{$reply['reply_id']}" class="btn btn-sm btn-light btn-icon btn-rounded">
                          <i class="fa fa-pencil-alt"></i>
                        </a>
      ...
      {elseif $view == "new-thread"}
        ...
        <ol class="breadcrumb forum-breadcrumb">
          ...
          {if $forum['parents']}
            ...
            {/foreach}
          {/if}
          <li class="breadcrumb-item"><a href="{$system['system_url']}/comunidade/forums/{$forum['forum_id']}/{$forum['title_url']}">{__($forum['forum_name'])}</a></li>
      ...
      {elseif $view == "edit-thread"}
        ...
        <ol class="breadcrumb forum-breadcrumb">
          ...
          {/if}
          <li class="breadcrumb-item"><a href="{$system['system_url']}/comunidade/forums/{$thread['forum']['forum_id']}/{$thread['forum']['title_url']}">{__($thread['forum']['forum_name'])}</a></li>
          <li class="breadcrumb-item"><a href="{$system['system_url']}/comunidade/forums/thread/{$thread['thread_id']}/{$thread['title_url']}">{$thread['title']}</a></li>
      ...
      {elseif $view == "edit-reply"}
        ...
        <ol class="breadcrumb forum-breadcrumb">
          ...
          {if $reply['thread']['forum']['parents']}
            ...
            {/foreach}
          {/if}
          <li class="breadcrumb-item"><a href="{$system['system_url']}/comunidade/forums/{$reply['thread']['forum']['forum_id']}/{$reply['thread']['forum']['title_url']}">{__($reply['thread']['forum']['forum_name'])}</a></li>
          <li class="breadcrumb-item"><a href="{$system['system_url']}/comunidade/forums/thread/{$reply['thread']['thread_id']}/{$reply['thread']['title_url']}">{$reply['thread']['title']}</a></li>
        ...
        {if $threads}
          {foreach $threads as $thread}
            <div class="forum-result">
              <div class="head">
                <div class="float-right">
                  {__("Forum")}: <a href="{$system['system_url']}/comunidade/forums/{$thread['forum']['forum_id']}/{$thread['forum']['title_url']}">{__($thread['forum']['forum_name'])}</a>
                </div>
              ...
              <div class="content">
                <div class="mb10">
                  <div class="mb5">
                    <strong class="title"><a href="{$system['system_url']}/comunidade/forums/thread/{$thread['thread_id']}/{$thread['title_url']}">{$thread['title']}</a></strong>
        ...
        {if $replies}
          {foreach $replies as $reply}
            <div class="forum-result">
              <div class="head">
                <div class="float-right">
                  {__("Forum")}: <a href="{$system['system_url']}/comunidade/forums/{$reply['thread']['forum']['forum_id']}/{$reply['thread']['forum']['title_url']}">{__($reply['thread']['forum']['forum_name'])}</a>
                </div>
              ...
              <div class="content">
                <div class="mb10">
                  <div class="mb5">
                    <strong class="title"><a href="{$system['system_url']}/comunidade/forums/thread/{$reply['thread']['thread_id']}/{$reply['thread']['title_url']}">{$reply['thread']['title']}</a></strong>
                    <div class="float-right text-right">
                  ...
                  <div class="mb5">
                    {__("By")}: <a href="{$system['system_url']}/comunidade/{$reply['thread']['user_name']}">{$reply['thread']['user_fullname']}</a> <span class="js_moment" data-time="{$reply['thread']['time']}">{$reply['thread']['time']}</span>
                  </div>
                </div>
                <div class="snippet">
      ...
      {elseif $view == "search"}
        ...
        <div class="card mt20">
          ...
          <form action="{$system['system_url']}/comunidade/forums/search-results" method="get">
            <div class="card-body">
              <div class="form-group form-row">
                <label class="col-md-2 form-control-label">
                  {__("Keyword(s)")}
