{if $_tpl == "box"}
  <li class="col-md-6 col-lg-3">
    <div class="ui-box">
      <div class="img">
        <a href="{$system['system_url']}/comunidade/events/{$_event['event_id']}{if $_search}?ref=qs{/if}">
          <img alt="{$_event['event_title']}" src="{$_event['event_picture']}" />
        </a>
      </div>
      <div class="mt10">
        <a class="h6" href="{$system['system_url']}/comunidade/events/{$_event['event_id']}{if $_search}?ref=qs{/if}">{$_event['event_title']}</a>
...
{elseif $_tpl == "list"}
  <li class="feeds-item">
    <div class="data-container {if $_small}small{/if}">
      <a class="data-avatar" href="{$system['system_url']}/comunidade/events/{$_event['event_id']}{if $_search}?ref=qs{/if}">
      ...
      <div class="data-content">
        ...
        <div>
          <span class="name">
            <a href="{$system['system_url']}/comunidade/events/{$_event['event_id']}{if $_search}?ref=qs{/if}">{$_event['event_title']}</a>
          </span>
          <div>{$_event['event_interested']} {__("Interested")}</div>
        </div>
      </div>
