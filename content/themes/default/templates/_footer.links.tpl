    ...
    <div class="col-sm-6 links">
      {if $static_pages}
        {foreach $static_pages as $static_page}
          <a {if !$static_page@first}class="pl10" {/if} href="{$system['system_url']}/comunidade/static/{$static_page['page_url']}">
            {__($static_page['page_title'])}
          </a>
        {/foreach}
      {/if}
      {if $system['contact_enabled']}
        <a class="pl10" href="{$system['system_url']}/comunidade/contacts">
          {__("Contact Us")}
        </a>
      {/if}
      {if $system['directory_enabled']}
        <a class="pl10" href="{$system['system_url']}/comunidade/directory">
          {__("Directory")}
        </a>
      {/if}
    </div>
