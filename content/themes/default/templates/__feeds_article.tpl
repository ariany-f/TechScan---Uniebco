{if $_tpl == "featured"}
  <div class="{if $_iteration == 1}col-sm-12 col-md-8 col-lg-6{else}col-sm-6 col-md-4 col-lg-3{/if}">
    <a href="{$system['system_url']}/comunidade/blogs/{$article['post_id']}/{$article['article']['title_url']}" class="blog-container {if $_iteration == 1}primary{/if}">
...
{else}
  <div class="post-media list">
    <div class="post-media-image-wrapper">
      <a class="post-media-image" href="{$system['system_url']}/comunidade/blogs/{$article['post_id']}/{$article['article']['title_url']}">
      ...
      <div class="post-media-image-meta">
        <a class="article-category {if $_small}small{/if}" href="{$system['system_url']}/comunidade/blogs/category/{$article['article']['category_id']}/{$article['article']['category_url']}">
          {__($article['article']['category_name'])}
        </a>
      </div>
    </div>
    <div class="post-media-meta">
      <a class="title mb5" href="{$system['system_url']}/comunidade/blogs/{$article['post_id']}/{$article['article']['title_url']}">{$article['article']['title']}</a>
      <div class="text mb5">
        {if $_small}
